...
function updateSingle($pdo, $r){
    ...
    $sql = 'UPDATE thuchi SET des=?, PID=?, amount=?, pom=? WHERE TCID=?';

 

    $stmt = $pdo->prepare($sql);

    $result = $stmt->execute(array($r->des, $r->PID, $r->amount, $r->pom, $r->TCID));
...
if( isset($_REQUEST['list'])){
    ...
    echo json_encode($dlist);die;
        ...
        $sql = "SELECT * FROM thuchi WHERE TCID > 100 ORDER BY TCID DESC";
